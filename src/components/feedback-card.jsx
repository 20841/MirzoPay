...
const FeedbackCard = ({ content, name, title }) => {
  return (
    <div className='flex justify-between flex-col px-10 py-12 rounded-[20px] max-w-[370px] md:mr-10 sm:mr-5 mr-0 my-5 cursor-pointer feedback-card'>
      <img src={quates} alt="" className="w-[42px] h-[27px] object-contain " />
    ...
    <div className="w-[50px] h-[50px] flex justify-center items-center bg-slate-50 rounded-full bg-dark-gradient">
        <p className="text-gradient text-[22px] font-semibold font-montserrat">
          {name.split(' ').map(c => c [0])
          .join('.').toUpperCase()}
        </p>
     </div>
